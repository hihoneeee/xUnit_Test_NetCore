...
namespace XUnitTestWebAPI.ServiceTests
{
    public class AuthServiceTests
    {
        ...
        {
            // Arrange
            var authRegisterDTO = new AuthRegisterDTO { phone = "12345678", password = "stringst", roleCode = "D22MD2", first_name = "Test", last_name = "test" };
            _authRepoMock.Setup(repo => repo.getByPhoneAsync(authRegisterDTO.phone)).ReturnsAsync((User)null);
            ...
            Assert.Equal("Phone already exists!", result.message);
        }
        ...
        {
            // Arrange
            var authRegisterDTO = new AuthRegisterDTO { phone = "12345678", password = "stringst", roleCode = "Invalid", first_name = "Test", last_name = "test" };
            _authRepoMock.Setup(repo => repo.getByPhoneAsync(authRegisterDTO.phone)).ReturnsAsync((User)null); // trả về null ở đây sẽ giả dụ không có
        ...
        {
            // Arrange
            var user = new User { phone = "0922712006", password = "stringst", first_name = "string", last_name = "string", roleCode = "D22MD2" };
            var role = new Role { value = "Admin", code = "D22MD2" };
            var authRegisterDTO = new AuthRegisterDTO { phone = "0922712006", password = "stringst", roleCode = "D22MD2", first_name = "string", last_name = "string" };
            _authRepoMock.Setup(repo => repo.getByPhoneAsync(authRegisterDTO.phone)).ReturnsAsync((User)null);
            _roleRepoMock.Setup(repo => repo.GetRoleByCodeAsyn(authRegisterDTO.roleCode)).ReturnsAsync(role);
            _authRepoMock.Setup(repo => repo.Register(It.IsAny<User>())).ReturnsAsync(user);
        ...
        {
            // Arrange
            var authLoginDTO = new AuthLoginDTO { phone = "0922712006" };
            _authRepoMock.Setup(repo => repo.getByEmail(authLoginDTO.phone)).ReturnsAsync((User)null);
        ...
        {
            // Assert
            var existingEmail = new User { phone = "0922712006", password = "stringst", first_name = "string", last_name = "string", roleCode = "D22MD2" };
            var authLoginDTO = new AuthLoginDTO { phone = "0922712006", password = "wrong_password" };

            _authRepoMock.Setup(repo => repo.getByEmail(authLoginDTO.phone)).ReturnsAsync(existingEmail);
        ...
        public async Task Login_Success_ReturnToken()
        {
            // Assert
            var existingEmail = new User { phone = "0922712006", password = "stringst", first_name = "string", last_name = "string", roleCode = "D22MD2" };
            var authLoginDTO = new AuthLoginDTO { phone = "0922712006", password = "stringst" };
